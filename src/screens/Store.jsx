...
import { genId, money } from "../lib/utils.js";
import { callClaude, parseAssistantReply } from "../lib/aiHelpers.jsx";
...
${JSON.stringify(shopContext)}`;
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-24 right-5 z-40 w-[94%] max-w-md bg-bg-surface rounded-2xl border border-white/10 shadow-2xl flex flex-col overflow-hidden" style={{ height: "78vh", maxHeight: 680 }}>
          <div className="flex items-center gap-2 px-4 py-3.5 border-b border-white/10 bg-indigo-600 text-white shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2.5">
            ...
            {loading && (
              <div className="flex items-center gap-1 px-3.5 py-2 bg-slate-800 border border-white/10 rounded-2xl rounded-bl-md w-fit">
                <Loader2 size={12} className="animate-spin text-indigo-500" />
                <span className="text-[10px] text-gray-400">Typing…</span>
              </div>
            )}
            {lastOrderId && (
              <div className="flex items-center gap-1.5 text-[11px] font-semibold text-green-400 bg-green-900/30 border border-green-500/20 rounded-xl px-3 py-2 w-fit">
          ...
          <div className="px-4 pb-2.5 flex flex-wrap gap-1.5 shrink-0">
            {suggestions.map((s) => (
              <button key={s} disabled={loading} onClick={() => send(s)} className="text-[10px] px-2.5 py-1.5 rounded-full bg-slate-800 border border-white/10 text-gray-300 hover:border-indigo-400 disabled:opacity-40">
                {s}
              </button>
            ))}
          </div>
...
export default function StoreScreen({ products, onBack, onLogin, placedOrders, onPlaceOrder }) {
  ...
  return (
    <div className="min-h-screen bg-bg-base" style={{ fontFamily: bodyFont }}>
      <div className="sticky top-0 z-30 bg-bg-surface border-b border-white/10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3.5 flex items-center gap-3">
          <button onClick={onBack} className="w-9 h-9 rounded-lg bg-slate-800 flex items-center justify-center text-gray-400 shrink-0 hover:bg-slate-700">
            <ArrowLeft size={16} />
          </button>
          <div className="flex items-center gap-2 font-bold text-text-primary" style={{ fontFamily: displayFont }}>
            <Sparkles size={16} className="text-indigo-400" /> AB Store
          </div>
          <div className="flex-1" />
          <div className="flex gap-1.5 bg-slate-800 p-1 rounded-xl">
            <button onClick={() => setView("browse")} className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition ${view === "browse" ? "bg-bg-surface text-indigo-400 shadow-sm" : "text-gray-400 hover:text-white"}`}>Shop</button>
            <button onClick={() => setView("tracking")} className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition ${view === "tracking" ? "bg-bg-surface text-indigo-400 shadow-sm" : "text-gray-400 hover:text-white"}`}>My Orders</button>
          </div>
          <button onClick={() => setCartOpen(true)} className="relative w-9 h-9 rounded-lg bg-indigo-900/30 text-indigo-400 flex items-center justify-center shrink-0 hover:bg-indigo-900/50">
            <ShoppingBag size={16} />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        {view === "browse" ? (
          <>
            <div className="mb-5">
              <h1 className="text-2xl font-bold text-text-primary mb-1" style={{ fontFamily: displayFont }}>Shop essentials, delivered fast.</h1>
              <p className="text-sm text-gray-400">Fresh groceries and daily needs from your neighbourhood store.</p>
            </div>
            <div className="flex gap-2 overflow-x-auto pb-1 mb-5">
              {cats.map((c) => (
                <button key={c} onClick={() => setCategory(c)} className={`text-xs font-semibold px-3.5 py-2 rounded-xl whitespace-nowrap transition ${category === c ? "bg-indigo-600 text-white" : "bg-bg-surface border border-white/10 text-gray-400 hover:bg-slate-800"}`}>
                  {c}
                </button>
              ))}
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {filtered.map((p) => (
                <div key={p.id} className="bg-bg-surface rounded-xl border border-white/10 shadow-sm p-4 flex flex-col hover:border-indigo-500/50 transition">
                  <div className={`w-full aspect-square rounded-lg flex items-center justify-center text-2xl font-bold mb-3 ${p.color}`}>
                    {p.name.slice(0, 1)}
                  </div>
                  <div className="text-xs font-semibold text-text-primary leading-snug mb-1 flex-1">{p.name}</div>
                  <div className="text-[11px] text-gray-400 mb-2">{p.category}</div>
          ...
          <div className="max-w-2xl">
            <h1 className="text-xl font-bold text-text-primary mb-4" style={{ fontFamily: displayFont }}>Order tracking</h1>
                ...
                {placedOrders.map((o) => (
                  <Card key={o.id}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-semibold text-text-primary text-sm">{o.id}</span>
                      <StatusBadge status={o.status} />
                    </div>
                    <div className="text-xs text-gray-400 mb-2">{o.items.map((it) => `${it.qty}x ${it.name}`).join(", ")}</div>
              ...
              {cartLines.map((l) => (
                <div key={l.productId} className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-semibold text-text-primary truncate">{l.product.name}</div>
                    <div className="text-[11px] text-gray-400">{money(l.product.price)}</div>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button onClick={() => updateQty(l.productId, -1)} className="w-6 h-6 rounded-md bg-slate-800 flex items-center justify-center text-gray-400 hover:bg-slate-700"><Minus size={11} /></button>
                    <span className="text-xs font-semibold w-5 text-center text-text-primary">{l.qty}</span>
                    <button onClick={() => updateQty(l.productId, 1)} className="w-6 h-6 rounded-md bg-slate-800 flex items-center justify-center text-gray-400 hover:bg-slate-700"><Plus size={11} /></button>
                  </div>
                </div>
              ))}
            </div>
            <div className="flex items-center justify-between text-sm font-bold pt-3 border-t border-white/10 mb-4">
      ...
      <Modal open={checkoutOpen} onClose={() => setCheckoutOpen(false)} title="Checkout" width={420}>
        ...
        <div className="flex items-center justify-between text-sm font-bold py-3 border-y border-white/10 mb-4">
      ...
      <Modal open={!!placed} onClose={() => setPlaced(null)} title="Order placed" width={380}>
        {placed && (
          <div className="text-center py-2">
            <div className="w-14 h-14 rounded-full bg-green-900/30 text-green-400 flex items-center justify-center mx-auto mb-3">
              <CheckCircle2 size={26} />
            </div>
            <div className="font-bold text-text-primary mb-1" style={{ fontFamily: displayFont }}>Thanks! Order {placed.id}</div>
            <p className="text-xs text-gray-400 mb-5">We've received your order and will confirm it shortly.</p>
