      ...
      <Modal open={!!placed} onClose={() => setPlaced(null)} title="Order placed" width={380}>
        {placed && (
          <div className="text-center py-2">
            ...
            <div className="font-bold text-slate-900 mb-1" style={{ fontFamily: displayFont }}>Thanks! Order {placed.id}</div>
            <p className="text-xs text-slate-500 mb-5">We've received your order and will confirm it shortly.</p>
...
function AdminApp({ onLogout, onGoStore, products, setProducts, orders, setOrders }) {
  const [section, setSection] = useState("dashboard");
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [notifOpen, setNotifOpen] = useState(false);
  const [customers] = useState(seedCustomers());
  const [campaigns] = useState(seedCampaigns());
  ...
  const handleUpdateStatus = (id, status) => setOrders((os) => os.map((o) => (o.id === id ? { ...o, status } : o)));

  const handleAddProduct = (form) => {
    setProducts((ps) => [
      { id: genId("P"), barcode: form.barcode || genId("BC"), name: form.name, category: form.category, price: Number(form.price) || 0, cost: Number(form.cost) || 0, stock: Number(form.stock) || 0, threshold: Number(form.threshold) || 10, color: "bg-indigo-100 text-indigo-700" },
      ...ps,
    ]);
  };
  const handleEditProduct = (id, form) => {
    setProducts((ps) => ps.map((p) => (p.id === id ? { ...p, ...form, price: Number(form.price) || 0, cost: Number(form.cost) || 0, stock: Number(form.stock) || 0, threshold: Number(form.threshold) || 10 } : p)));
  };
  ...
  const handlePOSCheckout = (cartLines) => {
    ...
    const total = cartLines.reduce((s, l) => s + l.subtotal, 0);
    setOrders((os) => [{ id: genId("ORD"), customer: "Walk-in customer", items: cartLines.map((l) => ({ name: l.product.name, qty: l.qty })), total, status: "delivered", date: new Date().toLocaleString(), channel: "POS" }, ...os]);
  };
    ...
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: bodyFont }}>
      ...
      <div className="lg:pl-64">
        ...
        <div className="p-4 sm:p-6 max-w-6xl">
          ...
          {section === "customers" && <CustomersView customers={customers} orders={orders} />}
          ...
          {section === "marketing" && <MarketingView campaigns={campaigns} />}
          {section === "assistant" && (
            <AssistantView
              orders={orders}
              products={products}
...
export default function BusinessAutomationSystem() {
  ...
  // adds it to the admin's order list, and to the shopper's own tracking list.
