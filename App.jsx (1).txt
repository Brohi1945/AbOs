...
function AdminApp({ onLogout, onGoStore, products, setProducts, orders, setOrders }) {
  ...
  const handleAddProduct = (form) => {
    const newProduct = { id: genId("P"), barcode: form.barcode || genId("BC"), name: form.name, category: form.category, price: Number(form.price) || 0, cost: Number(form.cost) || 0, stock: Number(form.stock) || 0, threshold: Number(form.threshold) || 10, color: "bg-indigo-100 text-indigo-700" };
