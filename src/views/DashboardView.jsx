...
import { money, todayLabel, computeWeeklyTrend } from "../lib/utils.js";
import { Card, Badge, StatCard, StatusBadge } from "../components/ui.jsx";
...
export default function DashboardView({ orders, products, customers, onGoTo }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-text-primary" style={{ fontFamily: displayFont }}>Good to see you 👋</h2>
        <p className="text-sm text-gray-400">{todayLabel()} — here's how the business is doing.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-bold text-text-primary text-sm" style={{ fontFamily: displayFont }}>Sales this week</h3>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={weeklyTrend}>
                ...
                />
                <Area type="monotone" dataKey="sales" stroke="#818CF8" strokeWidth={2.5} fill="url(#dashGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card noPad>
          <div className="flex items-center justify-between px-5 pt-5 mb-1">
            <h3 className="font-bold text-text-primary text-sm" style={{ fontFamily: displayFont }}>Low stock</h3>
            <button onClick={() => onGoTo("inventory")} className="text-[11px] font-semibold text-indigo-400 hover:text-indigo-300">View all</button>
          </div>
          {lowStock.length === 0 ? (
            <div className="px-5 py-8 text-xs text-gray-500 text-center">Everything is well stocked.</div>
          ) : (
      ...
      <Card noPad>
        <div className="flex items-center justify-between px-5 pt-5 mb-1">
          <h3 className="font-bold text-text-primary text-sm" style={{ fontFamily: displayFont }}>Recent orders</h3>
          <button onClick={() => onGoTo("orders")} className="text-[11px] font-semibold text-indigo-400 hover:text-indigo-300">View all</button>
        </div>
            ...
            <tbody>
              {recentOrders.map((o) => (
                <tr key={o.id} className="border-t border-white/5 hover:bg-white/5 transition">
                  ...
                  <td className="px-5 py-3 text-gray-300">{o.customer}</td>
                  <td className="px-5 py-3 text-text-primary font-medium">{money(o.total)}</td>
