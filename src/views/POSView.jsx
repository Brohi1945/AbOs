...
import { Card, Badge, Button, Modal, EmptyState } from "../components/ui.jsx";
...
export default function POSView({ products, onCheckout }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold text-text-primary" style={{ fontFamily: displayFont }}>POS / Retail mode</h2>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          <Card>
            <form onSubmit={handleScan} className="flex items-center gap-2">
              <div className="relative flex-1">
                <Barcode size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-indigo-400" />
                <input
                  ref={scanRef}
                  autoFocus
                  value={barcodeInput}
                  ...
                  className="w-full pl-10 pr-3.5 py-2.5 rounded-xl text-sm bg-indigo-900/30 border border-indigo-500/20 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-400 transition text-text-primary placeholder-gray-500"
                />
              </div>
              <Button type="submit">Add</Button>
            </form>
            {scanMsg && (
              <div className={`mt-2.5 text-xs font-semibold px-3 py-2 rounded-lg ${scanMsg.ok ? "bg-green-900/30 text-green-400" : "bg-red-900/30 text-red-400"}`}>
                {scanMsg.text}
              </div>
            )}
          </Card>
          ...
          <Card noPad>
            <h3 className="font-bold text-text-primary text-sm px-5 pt-5 mb-3" style={{ fontFamily: displayFont }}>Quick-add products</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 px-5 pb-5">
              {products.map((p) => (
                <button
                  ...
                  disabled={p.stock <= 0}
                  onClick={() => addToCart(p)}
                  className="text-left rounded-xl border border-white/10 p-3 hover:border-indigo-400 hover:shadow-sm transition disabled:opacity-40 disabled:hover:border-white/10"
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs mb-2 ${p.color}`}>{p.name.slice(0, 1)}</div>
                  <div className="text-xs font-semibold text-text-primary leading-tight mb-0.5">{p.name}</div>
                  <div className="text-[11px] text-gray-400">{money(p.price)} · {p.stock} in stock</div>
                </button>
              ))}
            </div>
          </Card>
        ...
        <Card noPad className="lg:sticky lg:top-20 h-fit">
          <div className="flex items-center justify-between px-5 pt-5 mb-1">
            <h3 className="font-bold text-text-primary text-sm" style={{ fontFamily: displayFont }}>Current bill</h3>
                ...
                {cartLines.map((l) => (
                  <div key={l.productId} className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-gray-400">{money(l.product.price)} each</div>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => updateQty(l.productId, -1)} className="w-6 h-6 rounded-md bg-slate-800 flex items-center justify-center text-gray-400 hover:bg-slate-700">
                        <Minus size={11} />
                      </button>
                      ...
                      <button onClick={() => removeLine(l.productId)} className="w-6 h-6 rounded-md hover:bg-red-500/10 flex items-center justify-center text-gray-500 hover:text-red-400">
                        <X size={12} />
                      </button>
                    </div>
                  </div>
        ...
        {invoice && (
          <div>
            <div className="text-center mb-4">
              <div className="font-bold text-text-primary" style={{ fontFamily: displayFont }}>AB OS</div>
              <div className="text-[11px] text-gray-400">{invoice.id} · {invoice.date}</div>
            </div>
            <div className="space-y-1.5 mb-4 pb-4 border-b border-dashed border-white/10">
              {invoice.lines.map((l) => (
                <div key={l.productId} className="flex items-center justify-between text-xs">
                  <span className="text-gray-400">{l.qty}x {l.product.name}</span>
                  <span className="font-semibold text-text-primary">{money(l.subtotal)}</span>
                </div>
              ))}
            </div>
