...
export function Sidebar({ active, onSelect, open, onClose, lowStockCount, onLogout, onGoStore }) {
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 bottom-0 w-64 bg-slate-950 border-r border-white/10 z-50 flex flex-col transition-transform lg:translate-x-0 ${
      ...
      >
        ...
        <div className="p-3 border-t border-white/10 space-y-1">
          <button
            onClick={onGoStore}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-400 hover:bg-slate-800 hover:text-indigo-400"
          >
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-400 hover:bg-slate-800 hover:text-red-400"
          >
  ...
  return (
    <div className="sticky top-0 z-30 bg-slate-900/80 backdrop-blur-md border-b border-white/10">
      <div className="flex items-center gap-3 px-4 sm:px-6 py-3.5">
        ...
        <button
          onClick={onMenuClick}
          className="lg:hidden w-9 h-9 rounded-lg bg-slate-800 border border-white/10 flex items-center justify-center text-gray-300"
        >
          <Menu size={17} />
        </button>
        ...
        <div className="flex-1 max-w-md relative hidden md:block">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 rounded-xl text-sm bg-slate-800 border border-white/10 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition"
          />
        </div>
        ...
        <div className="relative">
          <button
            ...
            className="relative w-9 h-9 rounded-lg bg-slate-800 border border-white/10 flex items-center justify-center text-gray-300"
          >
            <Bell size={16} />
          ...
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-72 bg-slate-900 rounded-xl border border-white/10 shadow-xl overflow-hidden">
              <div className="px-4 py-3 text-xs font-bold text-gray-300 border-b border-white/10">
                Notifications
              </div>
              ...
              {notifications.length === 0 ? (
                ...
                notifications.map((n, i) => (
                  <div key={i} className="px-4 py-2.5 text-xs border-b border-white/10 last:border-0">
                    <div className="font-semibold text-white">{n.title}</div>
                    <div className="text-gray-400">{n.note}</div>
                  </div>
                ))
              )}
            </div>
