...
/**
 * Computes each customer's live order count, total spent, and last order
 * date from the actual orders list, instead of relying on static seed
 * numbers. This means stats update immediately as new orders come in —
 * from POS, the storefront, or the AI assistant.
 */
...
function CustomersView({ customers, orders, onAdd }) {
  const [query, setQuery] = useState("");
  const [selected, setSelected] = useState(null);
 ...
 */
function computeWeeklyTrend(orders) {
  const days = [];
...
function MarketingView({ campaigns, onAdd }) {
  const [modalOpen, setModalOpen] = useState(false);
...
function AdminApp({ onLogout, onGoStore, products, setProducts, orders, setOrders, customers, setCustomers }) {
